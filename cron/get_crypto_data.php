...
$json = file_get_contents('https://api.coinbase.com/v2/prices/'.$currency_tag.'/buy');
$obj = json_decode($json);
$value = $obj->data->amount;

$myObj->amount = $value;
$myObj->timestamp = (intval(date('U')) * 1000) + 1;
$myJson = json_encode($myObj);

$section = '';
if($currency_tag === 'BTC-USD') {
	$section = 'bitcoin';
}
elseif ($currency_tag === 'LTC-USD') {
	$section = 'litecoin';
}
else {
	$section = 'ethereum';
...
$firebase->push('crypto_data/'.$section, $myObj);
echo "pushed crypto data";
