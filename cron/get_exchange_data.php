...
//Firebase connection code retrieved from https://github.com/ktamas77/firebase-php/tree/master/src
...
$url = 'URL HIDDEN FROM GITHUB';
$token = 'TOKEN HIDDEN FROM GITHUB';
$firebase = new FirebaseLib($url, $token);

$json = file_get_contents('https://api.coinbase.com/v2/exchange-rates');
$obj = json_decode($json);
$rates = $obj->data->rates;

foreach($rates as $key => $value) {
// Need to filter out certain keys from being added to firebase
	if($key === 'ZMK') {
		continue;
	}
	if($key === 'XPT') {
	...
	$updatedObj->exchange_rate = $value;
	$firebase->update('exchange/'.$key, $updatedObj);

}
echo "updated exchange data";
